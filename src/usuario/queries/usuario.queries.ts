const usuarioQueries = {
  ...
  selectLikeableUsers:
    'SELECT r.usuarioID, r.email, r.nombre, r.apellido FROM usuarios r WHERE r.activo = 1 AND NOT EXISTS (Select * FROM interacciones WHERE usuarioOrigenID = ? AND usuarioDestinoID = r.usuarioID ) AND r.email <> ?;',

  selectImagenes: 'SELECT * FROM usuario_imagenes WHERE usuario_ID = ?;',
  ...
  registerUser:
    'INSERT INTO usuarios (email, password, nombre, apellido, activo, rolID) VALUES (?, ?, ?, ?, ?, ?);',
  insertUserImage:
    'INSERT INTO usuario_imagenes (usuario_ID, imagen_url) VALUES (?, ?);',

  updatePassword: 'UPDATE usuarios SET password = ? WHERE email = ?;',
